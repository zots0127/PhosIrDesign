...
if [ -z "${OUTPUT_DIR+x}" ] || [ -z "${OUTPUT_DIR}" ]; then
  OUTPUT_DIR="Project_Output_${RUN_TIMESTAMP}"
  OUTPUT_DIR_AUTO=1
else
  OUTPUT_DIR_AUTO=0
fi
...
if [ -d "$TRAIN_DIR" ] && [ "$FORCE_TRAIN" = "0" ]; then
  ...
  python "$ROOT_DIR/scripts/cli/automl.py" train \
    data="$DATA_FILE" \
...
if [ "$SKIP_FIGURES" = "1" ]; then
  ...
  python "$ROOT_DIR/scripts/generate_publication_figures.py" \
    --project "$OUTPUT_DIR" \
    --data "$DATA_FILE" \
    --output "$OUTPUT_DIR/figures" 2>&1 | tee -a "$LOG_FILE" || warn "Publication figure generation failed"
fi
